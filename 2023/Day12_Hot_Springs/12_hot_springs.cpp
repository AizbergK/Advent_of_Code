...
int recursive_count(int test_iterator, long long *accumulator, std::string *test_string, std::vector<std::pair<int, int>> *numbers, int numbers_index) {
    ...
    if(is_not_last_in_numbers && ((*test_string)[substr_length + test_iterator] != '.') && ((*test_string)[substr_length + test_iterator] != '?')) {
        is_same = false;
    }
    ...
    if(is_same && is_not_last_in_numbers) {
        (*accumulator) += recursive_count(test_iterator + substr_length + 1, accumulator, test_string, numbers, numbers_index + 1);
    }
    if(test_iterator < (*numbers)[numbers_index].second) {
